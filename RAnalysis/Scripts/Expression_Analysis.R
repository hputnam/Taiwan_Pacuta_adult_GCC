...
#calculate nucleic acid and protein ratios
Data$RNA.DNA <- Data$RNA.microgram/Data$DNA.microgram
Data$Protein.DNA <- Data$Protein.microgram/Data$DNA.microgram
...
RNA.DNA <- aov(sqrt(RNA.DNA)~Temperature*CO2, data=Data)
RNA.DNA.s <- summary(RNA.DNA)
qqnorm(RNA.DNA$residuals)
qqline(RNA.DNA$residuals)
plot(RNA.DNA ~ interaction(Temperature,CO2), data=Data)

Protein.DNA <- aov(sqrt(Protein.DNA)~Temperature*CO2, data=Data)
Protein.DNA.s <- summary(Protein.DNA)
qqnorm(Protein.DNA$residuals)
qqline(Protein.DNA$residuals)
plot(Protein.DNA ~ interaction(Temperature,CO2), data=Data)
...
apx1 <- summary(aov(log10(apx1)~Temperature*CO2, data=Data))
qqnorm(apx1$residuals)
qqline(apx1$residuals)
plot(apx1 ~ interaction(Temperature,CO2), data=Data)

nrt2 <- summary(aov(log10(nrt2)~Temperature*CO2, data=Data))
...
capture.output(RNA.DNA.s, Protein.DNA.s, rbcl.s, pgpase.s, psI.s, symHSP.s, apx1.s, nrt2.s , file="/Users/hputnam/MyProjects/Taiwan_Pacuta_adult_GCC/RAnalysis/Output/gene.expression.stats.txt")
